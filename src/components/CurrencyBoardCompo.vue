<template>
    <div class="board">
        ...
        <div class="saisie">
            <label for="montant-xpf-compo">Montant à convertir</label>
...
const xpfSaisi = ref(1000)
const deviseSource = ref('XPF')
...
const erreurMontant = computed(() => {
    const v = xpfSaisi.value
...
const devisesAvecFlag = computed(() => {
    let tauxSource

    if (deviseSource.value === 'XPF') {
        tauxSource = 1
    } else {
        const deviseSelectionnee = devises.value.find(d => d.code === deviseSource.value)
...
onMounted(() => {
    chargerTaux()
    intervalId.value = setInterval(chargerTaux, 3600000)
})

// Stoppe l'intervalle quand le composant est détruit, sinon chargerTaux continuerait à s'exécuter.
onUnmounted(() => {
